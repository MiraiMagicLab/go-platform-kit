...
// Section represents an admin panel section exposed to the control plane.
type Section struct {
	ID           string            `json:"id"`
...
// Contract is a parsed admin contract payload.
...
// ParseContractSection unmarshals a raw JSON section as either a section ID string
// or a full [ContractSection].
...
// Compile builds a [Shell] from raw contract JSON and returns an error when the
// payload is invalid JSON.
func Compile(contractRaw json.RawMessage) (Shell, error) {
	if len(contractRaw) > 0 && !json.Valid(contractRaw) {
		return Shell{}, errors.New("admin: invalid contract JSON")
	}
	return buildShellFromContract(contractRaw), nil
}

// BuildShellFromContract builds a shell without surfacing JSON syntax errors.
// Prefer [Compile] for new code.
func BuildShellFromContract(contractRaw json.RawMessage) Shell {
	shell, _ := Compile(contractRaw)
	return shell
}
