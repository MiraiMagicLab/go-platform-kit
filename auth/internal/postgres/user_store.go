...
// Ensure UserAdapter implements store.UserRepository at compile time.
var _ store.UserRepository = (*UserAdapter)(nil)

// UserAdapter wraps *UserRepo to implement store.UserRepository.
...
func (a *UserAdapter) ListUsers(ctx context.Context, page, pageSize int, filter store.ListUsersFilter) ([]domain.User, int, error) {
	dtoFilter := ListUsersFilter{
