...
// MountAuth mounts authentication endpoints.
...
// MountEmail mounts email verification and password reset endpoints.
...
// MountOAuth mounts OAuth endpoints.
...
// MountRBAC mounts RBAC admin endpoints.
...
// registerAuthRoutes registers all auth routes (used by MountAuth and MountWithOptions).
func (m *Module) registerAuthRoutes(r gin.IRouter, opt AuthEndpoints) {
	if opt.Register {
		r.POST("/register", m.authH.Register)
	}
}
